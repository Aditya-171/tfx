...
class LatestPolicyModel(
  ...
  def apply(self, input_dict: typing_utils.ArtifactMultiMap):
    ...
    downstream_artifact_type_names_filter_query = (
        filter_query_builder.to_sql_string([
            ops_utils.MODEL_BLESSING_TYPE_NAME,
            ops_utils.MODEL_INFRA_BLESSSING_TYPE_NAME,
            ops_utils.MODEL_PUSH_TYPE_NAME,
        ])
    )
    input_child_artifact_ids_filter_query = filter_query_builder.to_sql_string(
    ...
    model_ids_to_downstream_artifacts = {}
    ...
    for id_index in range(0, len(model_artifact_ids), ops_utils.BATCH_SIZE):
      ...
      # Set `max_num_hops` to 100, as is limited by
      # `get_downstream_artifacts_by_artifact_ids()`.
      batch_model_ids_to_downstream_artifacts = (
      ...
      model_ids_to_downstream_artifacts.update(
          batch_model_ids_to_downstream_artifacts
      )
    ...
    ) in model_ids_to_downstream_artifacts.items():
