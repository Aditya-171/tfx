...
class ParsedSignature(NamedTuple):
  ...
  Attributes:
    ...
    outputs: A dictionary mapping each output name to its artifact type (as a
      subclass of `tfx.types.Artifact`).
    intermediate_outputs: A dictionary mapping each intermediate output artifact
      name to its artifact type (as a subclass of `tfx.types.Artifact`).
  ...
  intermediate_outputs: Dict[str, Type[artifact.Artifact]]
...
def _parse_signature(
  ...
  for arg in argspec.args:
    ...
    if isinstance(arg_typehint, annotations.InputArtifact):
      ...
      arg_formats[arg] = utils.ArgFormats.OUTPUT_ARTIFACT
      intermediate_outputs[arg] = arg_typehint.type
